...
这些用于基本的数学计算。假设我们有两个整数变量`a = 10`，`b = 3`。
...
用于给变量赋值。在赋值运算符中，我们需要强调**左值**（lvalue）和**右值**（rvalue）的概念。**左值**是指可以出现在赋值运算符左侧的表达式，它表示一个可修改的内存位置（如变量名），而**右值**是指出现在右侧的表达式，它是一个临时值或常量，不能被修改。例如，在`a = b + 1;`中，`a`是左值，`b + 1`是右值。
...
**章节练习**：
- 编写程序计算两个数的最大公约数（用%和循环）。
- 分析表达式 (a > b ? a : b) += 5 的含义。

继续下一章：《控制结构》，我们深入if、循环等。保持好奇，编程快乐！